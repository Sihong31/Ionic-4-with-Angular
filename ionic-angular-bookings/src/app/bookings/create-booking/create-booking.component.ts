...
export class CreateBookingComponent implements OnInit {
  ...
  @ViewChild('createBookingForm', {static: true}) form: NgForm;
  startDate: string;
  endDate: string;
  ...
  ngOnInit() {
    ...
    if (this.selectedMode === 'random') {
      ...
      this.endDate = new Date(
        ...
        .getTime() + Math.random() * (new Date(this.startDate).getTime() + 6 * 24 * 60 * 60 * 1000 - new Date(this.startDate).getTime())
        ).toISOString();
    }
  }
